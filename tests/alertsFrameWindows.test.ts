...
test.describe('Alerts frames and windows tests', () => {
  ...
  test.only("Alerts frame windows - Browser windows test", async ({ page, browserWindowsPage }) => {
    ...
    await newTab.close();
    const newWindow = await browserWindowsPage.getNewTab(browserWindowsPage.new_window_button);
    await expect((newWindow).locator('h1#sampleHeading')).toHaveText('This is a sample page');
    await newWindow.close();
    const newWindowMessage = await browserWindowsPage.getNewTab(browserWindowsPage.new_window_message_button);
  ...
  test("Alerts frame windows - Alert test", async ({ page, alertsPage }) => {
    await page.goto("/");
    await alertsPage.goToTab(alertsPage.alerts_frames_windows_card, alertsPage.alerts_tab);
  ...
  test("Alerts frame windows - frames test ", async ({ page, framesPage }) => {
    await page.goto("/");
    await framesPage.goToTab(framesPage.alerts_frames_windows_card, framesPage.frames_tab);
  ...
  test("Alerts frame windows - nested frames test ", async ({ page, nestedFramesPage }) => {
    await page.goto("/");
    await nestedFramesPage.goToTab(nestedFramesPage.alerts_frames_windows_card, nestedFramesPage.nested_frames_tab);
  ...
  test("Alerts frame windows - modal dialog test", async ({ page, modalDialogPage }) => {
    await page.goto("/");
    await modalDialogPage.goToTab(modalDialogPage.alerts_frames_windows_card, modalDialogPage.modal_dialogs_tab);
