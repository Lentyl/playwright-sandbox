...
test.describe('Elemenst section tests', () => {
  ...
  test("Elements - check box test", async ({ page, checkBoxPage }) => {
    await page.goto("/");
    await checkBoxPage.goToTab(checkBoxPage.elements_card, checkBoxPage.checkbox_tub);
  ...
  test("Elements - Radio button test", async ({ page, radioButtonPage }) => {
    await page.goto("/");
    await radioButtonPage.goToTab(radioButtonPage.elements_card, radioButtonPage.radio_button_tab);
  ...
  test("Elements - Web tables test", async ({ page, webTablesPage }) => {
    await page.goto("/");
    await webTablesPage.goToTab(webTablesPage.elements_card, webTablesPage.web_table_tab);
  ...
  test("Elements - Buttons test", async ({ page, buttonsPage }) => {
    await page.goto("/");
    await buttonsPage.goToTab(buttonsPage.elements_card, buttonsPage.buttons_tab);
  ...
  test("Elements - Links test", async ({ page, linksPage }) => {
    await page.goto("/");
    await linksPage.goToTab(linksPage.elements_card, linksPage.links_tab);
    ...
    expect((await linksPage.getNewTab(linksPage.home_link)).url()).toContain('https://demoqa.com/');
  })

  test("Elements - Broken links, images test", async ({ page, brokenImgeLinkPage }) => {
    await page.goto("/");
    await brokenImgeLinkPage.goToTab(brokenImgeLinkPage.elements_card, brokenImgeLinkPage.broken_link_image_tab);
  ...
  test("Elements - upload and download test", async ({ page, uploadAndDownloadPage }) => {
    await page.goto("/");
    await uploadAndDownloadPage.goToTab(uploadAndDownloadPage.elements_card, uploadAndDownloadPage.upload_and_download_tab);
    await uploadAndDownloadPage.downloadFile();
    expect(await uploadAndDownloadPage.isFileWasDownloaded('data/downloads/sampleFile.jpeg')).toBe(true);
    await uploadAndDownloadPage.uploadFile();
    await uploadAndDownloadPage.deleteFile('data/downloads/sampleFile.jpeg');
  ...
  test("Elements - dynamic properties test", async ({ page, dynamicPropertiesPage }) => {
    await page.goto("/");
    await dynamicPropertiesPage.goToTab(dynamicPropertiesPage.elements_card, dynamicPropertiesPage.dynamic_Properties_tab);
