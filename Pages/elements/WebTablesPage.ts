...
export default class WebTablesPage extends BasePage {
  ...
  async addUser(userQuantity: number = 0) {
    for (let i = 0; i <= userQuantity; i++) {
      ...
      await this.first_name_input.fill(testData.first_names[(await this.randomNumber(0, testData.first_names.length - 1))]);
      await this.last_name_input.fill(testData.last_names[(await this.randomNumber(0, testData.last_names.length - 1))])
      await this.email_input.fill(`mar${new Date().getTime()}@gmail.com`);
      await this.age_input.fill(await this.randomNumber(18, 70) + "");
      await this.salary_input.fill(await this.randomNumber(2000, 6000) + "");
      await this.department_input.fill(testData.department[await this.randomNumber(0, testData.department.length - 1)]);
